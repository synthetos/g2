 ...
 *       i. RUNNING/COMMAND         COMMAND(s)          BACK_PLANNED/MOVE   skip/plan command(s), plan move (Note 1a),
 * exit OK j. RUNNING/COMMAND         COMMAND(s)          FULLY_PLANNED/MOVE  skip command(s), exit NOOP
 *
...
stat_t mp_exec_move() {
    ...
    if (bf->block_type == BLOCK_TYPE_ALINE) {  // cycle auto-start for lines only
        ...
        if (bf->buffer_state != MP_BUFFER_RUNNING) {
            ...
            if (bf->buffer_state == MP_BUFFER_BACK_PLANNED) {
                // debug_trap_if_true((cm->motion_state == MOTION_RUN), "mp_exec_move() buffer prepped but not
                // planned"); IMPORTANT: can't rpt_exception from here! We need to have it planned. We don't want to do
                // this here, as it might already be happening in a lower interrupt.
                st_prep_null();
    ...
    if (mr->block_state == BLOCK_INACTIVE) {
        ...
        for (uint8_t axis = 0; axis < AXES; axis++) {
            ...
            mr->waypoint[SECTION_BODY][axis] =
                mr->position[axis] + mr->unit[axis] * (mr->r->head_length + mr->r->body_length);
            mr->waypoint[SECTION_TAIL][axis] =
                mr->position[axis] + mr->unit[axis] * (mr->r->head_length + mr->r->body_length + mr->r->tail_length);
        }
    }
...
static stat_t _exec_aline_segment() {
    ...
    kn->inverse_kinematics(mr->gm, mr->gm.target, mr->position, mr->segment_velocity, mr->target_velocity,
                           mr->segment_time, exec_target_steps);

    // Update the mb->run_time_remaining -- we know it's missing the current segment's time before it's loaded, that's
    // ok.
    ...
    if (cm->hold_state == FEEDHOLD_MOTION_STOPPING) {
        if (mp_runtime_is_idle()) {  // wait for steppers to actually finish
            ...
            // Otherwise setup the block to complete motion (regardless of how hold will ultimately be exited)
                ...
                // If length ~= 0 it's because the deceleration was exact. Handle this exception to avoid planning
                // errors
                if (bf->length < EPSILON4) {
                    ...
                    while (bf->buffer_state > MP_BUFFER_BACK_PLANNED) {
                        bf->buffer_state =
                            MP_BUFFER_BACK_PLANNED;  // revert from RUNNING so it can be forward planned again

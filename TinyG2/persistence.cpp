...
{
	if (cm.cycle_state != CYCLE_OFF) return(rpt_exception(STAT_FILE_NOT_OPEN, "write_persistent")); // can't write when machine is moving
...
stat_t write_persistent_value(nvObj_t *nv)
{
	...
	if (read_persistent_value(nv) != STAT_OK ||
		...
		((uint32_t)nv->value != (uint32_t)nvm.tmp_value)) { // use a bitwise equality check rather than fp_EQ

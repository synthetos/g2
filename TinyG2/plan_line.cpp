...
float mp_get_runtime_work_position(uint8_t axis) {
    ...
    if (axis == AXIS_X) {
        return mr.position[0] * cm.rotation_matrix[0][0] +
               mr.position[1] * cm.rotation_matrix[1][0] +
               mr.position[2] * cm.rotation_matrix[2][0] -
               mr.gm.work_offset[0];
    }
    else if (axis == AXIS_Y) {
        return mr.position[0] * cm.rotation_matrix[0][1] +
               mr.position[1] * cm.rotation_matrix[1][1] +
               mr.position[2] * cm.rotation_matrix[2][1] -
               mr.gm.work_offset[1];
    }
    else if (axis == AXIS_Z) {
        return mr.position[0] * cm.rotation_matrix[0][2] +
               mr.position[1] * cm.rotation_matrix[1][2] +
               mr.position[2] * cm.rotation_matrix[2][2] -

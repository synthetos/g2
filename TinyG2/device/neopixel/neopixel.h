...
// Remember, I (intensity) is, effectively, brightness. Unlike L (lightness) and V (value), the hue is visible for all I.
...
struct HSI_Color_t : NeopixelColorTag {
    ...
    HSI_Color_t(uint32_t _update_every_ms = 1) : _update_timeout_ms{_update_every_ms}
    {
        _update_timeout.set(0);
    };

    HSI_Color_t(float _hue, float _saturation, float _intensity, uint32_t _update_every_ms = 1) : hue{_hue}, saturation{_saturation}, intensity{_intensity}, _update_timeout_ms{_update_every_ms}
    {
        _update_timeout.set(0);
    };
    ...
    void startTransition(uint32_t milliseconds, float to_hue, float to_saturation, float to_intensity) {
        ...
        if (fabs(hue - to_hue) > fabs(hue - (360.0 + to_hue))) { to_hue += 360.0; }
    ...
    bool update()
    {
        if (!_update_timeout.isPast()) { return false; }
    ...
    void getRGB(uint8_t &r, uint8_t &g, uint8_t &b) {
        ...
        if (intensity > 1.0) { saturation = 1.0; }
...
struct RGB_Color_t : NeopixelColorTag {
    ...
    RGB_Color_t(uint32_t _update_every_ms = 1) : _update_timeout_ms{_update_every_ms}
    {
        _update_timeout.set(0);
    };

    RGB_Color_t(float _red, float _green, float _blue, uint32_t _update_every_ms = 1) : red{_red}, green{_green}, blue{_blue}, _update_timeout_ms{_update_every_ms}
    {
        _update_timeout.set(0);
    };
...
struct NeoPixel {
    ...
    template <typename  color_t, typename std::enable_if<std::is_base_of<NeopixelColorTag, color_t>::value, NeopixelColorTag>::type* = nullptr>
